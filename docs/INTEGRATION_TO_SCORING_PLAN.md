...
P0–P4 已通过。Task 3 独立 `--apply` 已通过（去掉进柜前内收后的复验也成功）。**下一步：重启 Server 恢复开局布局，做固定布局三任务连续闭环（P5）。** 需要 Client 容器，不要启动不带 `--preflight-only` 的 `formal_client.py`。详情见 `CURRENT_STATUS.md` 第 15 节。
...
任务：顶部取物、避开白色支撑物、从 L2 板下钻入 L1、相对 `packaging_box` 左侧放置。独立脚本 `task3_cube_top_shelf_place_check.py` 已真实通过（2026-08-28，含去掉进柜前内收后的复验）。当前代码要点：

```text
place_world 派生  [-2.68, 0.5685, 0.498]   立柱内侧与包装南面的中点，不是贴柱的 0.54
grasp_fwd         0.06 m                   掌包住侧面
插入              柜前南移 → 正西进 L1     朝向误差 ≤ 0.02 rad
松手              左右各 0.04 m + 抽指 0.04 m，然后倒车，最后收臂
不要              柜内拆抱持、进柜前把掌往回抽（会 settle 失败并把物体碰掉）
```

P5 需要开局布局，先重启 Server 再按 Task 1→2→3 连续跑。不要用 Task 1 的 grasp_fwd/z 替代 Task 3。
...
P0 站位 passed
  ...
  -> Task 3 独立 --apply passed（2026-08-28；08:51 内收失败已撤销，其后复验成功）
  -> 当前：重启 Server 后做固定布局三任务连续闭环（P5）
  -> 固定布局三任务连续闭环
